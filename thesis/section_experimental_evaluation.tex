...
\paragraph{Effect of the search refinements.}
\begin{itemize}
...
therefore say little about how reliable a reported difference is. Since almost
all comparisons are paired, the same positions are measured under both
configurations, uncertainty is quantified on the paired differences
$d_i = x_{i,A} - x_{i,B}$ instead. Reported intervals are $95\,\%$ percentile
bootstrap confidence intervals over $10\,000$ resamples, drawn over the unit that
is actually independent: positions for the suite experiments, and complete games
for the tree-reuse experiment, whose plies are clustered within games. An
interval that excludes zero is called a difference; one that does not is reported
as such, without claiming that no effect exists.
...
Two caveats apply to the reported interval. The games are played as
...
10   & 3.67 & 3.91 & $+0.23$ [$+0.19$, $+0.28$] & 60.2 & 61.9 & $+1.7$ [$-1.6$, $+4.9$] \\
25   & 4.29 & 4.65 & $+0.36$ [$+0.31$, $+0.41$] & 58.9 & 60.7 & $+1.8$ [$-2.1$, $+5.5$] \\
50   & 4.86 & 5.30 & $+0.44$ [$+0.39$, $+0.50$] & 57.1 & 56.0 & $-1.1$ [$-8.9$, $+6.7$] \\
100  & 5.37 & 5.96 & $+0.59$ [$+0.52$, $+0.66$] & 57.2 & 60.2 & $+3.0$ [$-5.2$, $+11.8$] \\
250  & 6.04 & 6.81 & $+0.78$ [$+0.70$, $+0.85$] & 54.6 & 55.7 & $+1.1$ [$-7.5$, $+8.5$] \\
500  & 6.52 & 7.53 & $+1.02$ [$+0.95$, $+1.09$] & 56.8 & 55.0 & $-1.9$ [$-8.1$, $+3.8$] \\
1000 & 7.08 & 8.16 & $+1.09$ [$+1.02$, $+1.17$] & 55.5 & 52.4 & $-3.1$ [$-9.3$, $+2.5$] \\
2000 & 7.55 & 8.79 & $+1.25$ [$+1.17$, $+1.33$] & 49.2 & 50.3 & $+1.1$ [$-9.7$, $+11.4$] \\
5000 & 8.09 & 9.43 & $+1.35$ [$+1.27$, $+1.43$] & 43.8 & 44.4 & $+0.6$ [$-4.3$, $+5.1$] \\ \bottomrule
\end{tabular}
\caption{Reachable depth and centipawn loss with and without null-move pruning,
over 309 positions and three repetitions per budget. $\Delta$ is the paired
difference (NMP minus no NMP) with a 95\,\% bootstrap confidence interval over
positions; positive values mean more depth respectively higher loss.}
...
saturates at one second, the mean still falls from $52$ to $44$\,cp between one
and five seconds, because a minority of positions continues to improve.
\cref{subsec:eval_outliers} shows that this minority dominates the mean, so the
honest formulation is that \emph{typical} move quality saturates around one
second on this suite, while \emph{average} quality keeps improving through a
small group of difficult positions.
...
A caveat applies to this sweep specifically: with 30 configurations compared
against one another, some differences of this size are expected by chance alone,
and no correction for multiple comparisons was applied. The result should be read
as the absence of a usable tuning gain, not as a precise ranking of the
configurations.
...
versus a full ply), but it is obtained differently: no search parameter is
changed, no subtree is discarded, and no accuracy is traded. The engine merely
stops throwing away what it has already computed.
