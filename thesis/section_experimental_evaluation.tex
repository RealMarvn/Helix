...
conditions. Matches are played through the CuteChess framework, with Stockfish as
the reference opponent restricted through its built-in strength limitation
(\texttt{UCI\_LimitStrength}), a single search thread, and a hash size of
32\,MB. Both engines play a classical time control of five minutes per side with
a 0.5\,second increment, without an opening book and with alternating colors so
that each side plays an equal number of games as White and Black. Playing
strength is reported as an Elo difference derived from the match result, together
with the likelihood of superiority and the draw ratio.

\emph{Note: the match results from the earlier Bachelor-project setup are no
longer current and are being re-measured with the present build; the final
...
A further question is at which depth the engine's chosen move stabilizes, i.e.,
the depth beyond which additional iterations no longer change the root decision.
This is measured by recording, for each position, the best move at every
completed iteration of iterative deepening and determining the depth from which
it no longer changes.

\emph{Note: this experiment is still being measured with the current build; the
results and the corresponding table will be inserted here.}
