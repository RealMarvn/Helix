...
is null-move pruning, introduced by Beal~\cite{beal1989nullmove}: if passing a
turn still leaves the position good enough to cause a cutoff, the subtree is
assumed to be safe to skip. Unlike Alpha-Beta this is not value-preserving, so it
trades a small risk of overlooking a line for a substantially smaller tree.
...
ply to decrease with depth, an effect partly masked by the high error rate of
programs at shallow depths. Ferreira~\cite{ferreira2013impact} approached the
