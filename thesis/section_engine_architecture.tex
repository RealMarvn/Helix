...
supports incremental updates during move making, the implementation used in this
engine recomputes the hash from the board state when a move is applied. Undoing a
move needs no recomputation at all: the hash is stored alongside the board
settings in the move history and is restored from there. The technique itself was
originally introduced by Zobrist~\cite{zobrist1970hashing} and has since become a
standard component of modern chess engines.

One detail of the hash deserves mention because it directly affects the hit rate
of the table. The en-passant target square is part of the hashed state, so a
position in which a pawn has just advanced two squares hashes differently from
the otherwise identical position without that pawn move. Setting the target
square after every double step would therefore split positions that are in fact
equivalent, since en passant is only ever relevant when an enemy pawn is actually
positioned to capture. The engine consequently only records the target square
when a capturing pawn stands next to the advanced pawn. Measured over the
benchmark suite, this raises the transposition table hit rate from $0.111$ to
$0.118$ at a one-second budget.
