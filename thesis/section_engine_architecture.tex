...
The implemented chess engine follows a layered architecture whose structural
design decisions and component interactions are the focus of this subsection; the
rationale behind these decisions is summarized at the end.
...
\node[box, right=0.5cm of search] (heuristics) {Heuristics - Move ord.\\(PV/TT+HH+KM+Material)};
...
\subsubsection{Game State and Move Handling}
...
It is worth being explicit about what this decision costs, since it is the single
most consequential one in the architecture. The engine sustains roughly
$1.7$ million nodes per second and reaches an average depth of $8.2$ plies at a
one-second budget (\cref{subsec:eval_ttq}). A bitboard implementation would
plausibly search several times as many nodes per second, but because depth grows
only logarithmically with search effort, even a fivefold faster move generator
would buy no more than about $1.5$ additional plies at the same budget. The
representation therefore shifts the operating point of the measurements without
changing their structure: every result in this thesis is a statement about how
decision quality behaves \emph{as a function of} depth and budget, and such a
statement is displaced along the axis by a faster engine, not invalidated. The
same reasoning is the reason absolute playing strength is treated throughout as a
sanity check rather than as a result.
...
\subsubsection{Position Hashing and Transposition Tables}
...
positions to be mapped efficiently to fixed-size keys. The technique supports
incremental updates during move making, where only the keys of the affected
squares are combined into the existing hash. This engine instead recomputes the
hash from the board state whenever a move is applied, which keeps the hash
independent of the move-making code: castling, en-passant and promotion each
modify several squares at once, and an incremental update has to mirror every one
of these cases correctly, whereas a full recomputation cannot fall out of sync
with the board. Undoing a move needs no recomputation at all, since the hash is
stored alongside the board settings in the move history and restored from there.
The technique itself was originally introduced by
Zobrist~\cite{zobrist1970hashing} and has since become a standard component of
modern chess engines.

Note that the en-passant target square is part of the hashed state, so a position
in which a pawn has just advanced two squares hashes differently from the
otherwise identical position without that pawn move. Setting the target square
after every double step would therefore split positions that are in fact
equivalent, since en passant is only relevant when an enemy pawn is actually
positioned to capture. The engine consequently only records the target square
when a capturing pawn stands next to the advanced pawn, which raises the
transposition table hit rate from $0.111$ to $0.118$ at a one-second budget over
the benchmark suite.
...
preventing the table from being dominated by stale information. Since the table
persists across search invocations, it additionally serves as the mechanism by
which the engine resumes previously explored search trees, as described in
\cref{subsec:tree_reuse}. For practical
implementations and further discussion, see for example Warnock~\cite{warnock1988search}.

\subsubsection{Search Control and Integration}
...
\subsubsection{Universal Chess Interface (UCI)}
\label{subsec:uci}

Externally, the engine is controlled through the \emph{Universal Chess Interface}
(UCI), the most widely used communication protocol in modern computer chess. UCI
specifies a text-based command interface that allows external tools to set up
positions, initiate searches, and receive search results; search constraints such
as time limits, depth limits, node limits, and pondering are communicated via the
\texttt{go} command.

The protocol itself is independent of the engine's internal implementation and
therefore separates external control from internal search logic. This separation
is reflected directly in the design described above: incoming UCI parameters are
first mapped to the abstract \texttt{SearchConstraints} representation
(\cref{sec:time_management}) before being enforced during search. All experiments
reported in this work are conducted through UCI-compatible interfaces and
tournament frameworks, which makes UCI the practical context in which
time-constrained search and iterative deepening are evaluated.
...
time-dependent decision making rather than to compete directly with
top-performing chess engines.