...
\begin{itemize}
    \item \textbf{FixedDepth:} the time budget is explicitly disabled by leaving
    \texttt{budget\_} empty, which prevents time-based termination inside
...
obtained from \texttt{get\_legal\_\allowbreak fallback\_move()}.

\subsubsection{Iterative Deepening Loop}

Time-managed searches run in \texttt{ChessBot::\allowbreak iterative\_deepening(Board\&)}.
...
To support correct transposition table storage, the implementation keeps the
original alpha value in \texttt{ORIGINAL\_ALPHA} and uses it afterwards to
classify the result: as \texttt{EXACT} if the score lies inside the window, as
\texttt{LOWER\_BOUND} if the node failed high, and otherwise as
\texttt{UPPER\_BOUND}.
...
Quiescence search is implemented in \texttt{ChessBot::\allowbreak quiescence(...)} and is
...
whole subtree can be pruned without a full-depth search. The recursive form with
depth reduction implemented here follows Goetsch and
Campbell~\cite{goetsch1990experiments} and is a standard component of classical
engines.
...
Move ordering is implemented in \texttt{search::heuristics::\allowbreak order\_moves}.
