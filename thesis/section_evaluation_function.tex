...
Since the search explores only a limited portion of the game tree, a heuristic
evaluation is required to estimate the quality of non-terminal positions. The
implemented evaluation follows a classic \emph{tapered} approach with separate
midgame (MG) and endgame (EG) scores, piece-square tables (PSQT), a phase-based
interpolation scheme, and a tempo bonus. Its material values and tables are
adapted from the widely used PeSTO tables~\cite{cpw_pesto}.
...
\cref{fig:tapered_eval} shows the two weights over the course of a game: as
material leaves the board, the endgame table gradually takes over from the
midgame table, so that a king which was penalized for leaving its shelter early
on is rewarded for centralizing later, without either table ever switching on
abruptly.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=1.0]
  \draw[->] (0,0) -- (8.2,0) node[below right] {game phase (normalized)};
  \draw[->] (0,0) -- (0,4.2) node[above left] {weight};
...
\cref{algo:evaluation} combines the parts described above into the procedure the
engine actually executes. A single pass over the 64 squares accumulates the
midgame and endgame scores for both sides together with the game phase, so that
material, piece-square values and phase are collected in one traversal. The two
scores are then formed from the perspective of the side to move, interpolated
according to the phase, and the tempo bonus is added.
...
The first of these is the one that turns out to matter most in practice. Since
neither passed pawns nor pawn structure are represented, two moves that leave
material untouched can be scored almost identically even when one wins and the
other draws. \cref{subsubsec:eval_case_studies} shows a position in which exactly
this happens: both candidate moves are pawn pushes on the same file, and the
evaluation has no term that could prefer the one advancing a passer over the one
advancing a pawn into its own blockade.
