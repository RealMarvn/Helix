\subsection{Time Management and Constraints}
...
\subsubsection{Unified Constraint Model}
...
\subsubsection{Time Budgets: Structure and Arming}

Time-based searches (Normal / FixedTime) rely on \texttt{TimeControl} stored in
\texttt{SearchConstraints::\allowbreak budget\_}. The budget distinguishes a \emph{soft} and a
\emph{hard} limit:
\begin{itemize}
...
Immediately before a time-managed search begins, the engine calls
\texttt{TimeManager::init\_search}. This function captures the current monotonic time
...
\subsubsection{Termination Conditions in the Search}
...
\If{\texttt{budget\_.\allowbreak hard\_time\_up(now)}}{
    \Return \texttt{true}\;
}
...
\For{$d \gets 1,2,3,\dots$}{
    ...
    \If{\texttt{budget\_.\allowbreak soft\_time\_up(now)}}{
