...
struct Move
{
    ...
    /**
     * @brief Converts the Move object's square and moveSquare coordinates to corresponding X and Y
     * coordinates.
     *
     * This function takes the square and moveSquare coordinates of a Move object and converts them
     * to X and Y coordinates. The X coordinate is obtained by adding the ASCII value of 'a' to the
     * remainder of square divided by 8, and casting it to a character. The Y coordinate is obtained
     * by adding 1 to the result of square divided by 8. The same process is applied to the
     * moveSquare coordinates to obtain the second set of X and Y coordinates. The resulting X and Y
     * coordinates are then concatenated together as a string and returned.
     *
    ...
    bool operator==(const Move& other) const
    {
        return move_square == other.move_square && square == other.square &&
               ...
               promotion_piece.piece_type == other.promotion_piece.piece_type &&
 ...
 *
 * The PseudoLegalMoves class provides a container for storing pseudo-legal chess moves. It uses a
 * fixed-size array to store the moves and provides methods for accessing and modifying the move
 * list.
 */
class PseudoLegalMoves
{
  ...
  public:
     ...
     *
     * This function returns an iterator pointing to one past the last element in the move list. It
     * is used to indicate the end of the range of elements in the move list.
     *
     * @return An iterator pointing to one past the last element of the move list.
     */
    iterator end()
    {
        return move_list.begin() + index;
    }
     ...
     *
     * This function adds a move to the move list by assigning it to the element at the current
     * index. It then increments the index by 1.
     *
     ...
     *
     * This function checks if the move list contains the specified move. It uses the std::find
     * algorithm to search for the move in the move list. If the move is found, it returns true;
     ...
     * The lambda function compares the scores of two moves and returns true if the score of the
     * left move is greater than the score of the right move. mvv-lva = most valuable victim, least
     * valuable attacker
     *
     * @return None.
     */
  ...
  private:
     ...
     *
     * This function calculates the score of a given move based on the piece types involved in the
     * move. The score is calculated by taking the piece type of the captured piece (modulo BP = 6
     * for no color piece) and multiplying it by 10, and then subtracting the piece type of the
     * moving piece (modulo BP = 6 for no color piece).
     *
