...
static void getAllLinearMoves(std::pair<int, int>& start_square, Board& board, PseudoLegalMoves& all_pseudo_moves,
                              const bool PIECE_COLOR, const PieceType MOVING_PIECE,
  ...
  for (const auto& dir : directions) {
    ...
    while (x > 0 && y > 0 && x < 9 && y < 9) {
      ...
      if (board[POSITION].piece_type == EMPTY) {
        ...
        if ((PIECE_COLOR && board[POSITION].is_white()) || (!PIECE_COLOR && !board[POSITION].is_white())) {
...
void moveGenUtils::get_all_possible_rook_moves(std::pair<int, int> start_square, Board& board,
  ...
  constexpr std::array<std::pair<int, int>, 4> DIRECTIONS = {std::pair(-1, 0), std::pair(1, 0), std::pair(0, -1),
                                                             std::pair(0, 1)};

  // Generate all the moves.
  getAllLinearMoves<4>(start_square, board, all_pseudo_moves, PIECE_COLOR, (PIECE_COLOR ? WR : BR), DIRECTIONS);
}
...
void moveGenUtils::get_all_possible_bishop_moves(std::pair<int, int> start_square, Board& board,
  ...
  constexpr std::array<std::pair<int, int>, 4> DIRECTIONS = {std::pair(-1, -1), std::pair(-1, 1), std::pair(1, -1),
                                                             std::pair(1, 1)};
  // Generate all the moves.
  getAllLinearMoves<4>(start_square, board, all_pseudo_moves, PIECE_COLOR, (PIECE_COLOR ? WB : BB), DIRECTIONS);
}
...
void moveGenUtils::get_all_possible_queen_moves(std::pair<int, int> start_square, Board& board,
  ...
  // Generate all the moves.
  getAllLinearMoves<8>(start_square, board, all_pseudo_moves, PIECE_COLOR, (PIECE_COLOR ? WQ : BQ), DIRECTIONS);
}

void moveGenUtils::get_all_possible_king_moves(const std::pair<int, int>& START_SQUARE, Board& board,
                                               PseudoLegalMoves& all_pseudo_moves, const bool PIECE_COLOR) {
  ...
  if (!board.is_king_in_check(PIECE_COLOR)) {
    ...
    if ((PIECE_COLOR && board.board_settings.white_king_side) ||
      ...
      for (int i = 1; i < 3; i++) {
        ...
        if (board[position].piece_type != EMPTY || board.is_square_attacked({X + i, y}, PIECE_COLOR)) {
...
void moveGenUtils::get_all_possible_knight_moves(const std::pair<int, int>& START_SQUARE, Board& board,
                                                 PseudoLegalMoves& all_pseudo_moves, const bool PIECE_COLOR) {
...
void moveGenUtils::get_all_possible_pawn_moves(const std::pair<int, int>& START_SQUARE, Board& board,
                                               PseudoLegalMoves& all_pseudo_moves, const bool PIECE_COLOR) {
  ...
  for (const auto& [FST, SND] : directions) {
    ...
    if (x > 0 && y > 0 && x < 9 && y < 9) {
      ...
      if (FST != 0 && SND != 0) {
        ...
        if (piece.piece_type != EMPTY) {
          ...
          if ((PIECE_COLOR && y == 8) || (!PIECE_COLOR && y == 1)) {
            ...
            for (int promotionIndex = 0; promotionIndex < 4; promotionIndex++) {
              move.promotion_piece.piece_type = (PIECE_COLOR ? white_pawn_possible_promotions[promotionIndex]
          ...
          if (board.board_settings.ep_square != 100 && calculateSquare(x, y) == board.board_settings.ep_square) {
            // Check for EP
        ...
        if (SND == 2) {
          if ((PIECE_COLOR && START_SQUARE.second == 2) || (!PIECE_COLOR && START_SQUARE.second == 7)) {
            // Check if allowed to do the double jump.
            if (board[calculateSquare(x, START_SQUARE.second + (PIECE_COLOR ? 1 : -1))].piece_type == EMPTY) {
